...
def startScreen():
    global canvas, enabled, game, loop, file
    game = True
    loop = True
    canvas = Canvas(window, width=w, height=h)
    bg_label = Label(window, image=file)
...
def gameArena():
    ...
    global keyUp, keyDown, keyRight, keyLeft, startTime, endTime
    paused = False
    direction1 = "right"
    direction2 = "left"
    ...
    window.update()
    sleep(1)  # Wait 1 second before the game starts
...
def gameLoop():
    ...
    global userGame, startTime, endTime, seconds
    newGame = True
...
def difficulty():
    ...
    btn = Button(window, text="Medium", command=medium,
                 ...
                 bg="#2794a1", font=("Helvetica 15"),
                 pady="5", bd=5, relief="flat")
    btn.place(x=806, y=500)
    btn = Button(window, text="Hard", command=hard,
                 width=25, height=2,
                 bg="#2794a1", font=("Helvetica 15"),
                 pady="5", bd=5, relief="flat")
    btn.place(x=806, y=600)
...
def controls():
    ...
    userInputDown.place(x=806, y=440)
    ...
    userInputRight.place(x=806, y=530)
    ...
    userInputLeft.place(x=806, y=620)
...
def userInputApply():
    ...
    if (len(keyUp) > 1 or len(keyDown) > 1 or
        ...
        successLabel.place(x=756, y=100)
        window.update()
        sleep(2)
...
window.after(5000, startScreen)  # Start Main Menu after 1 second
loadingScreen()
